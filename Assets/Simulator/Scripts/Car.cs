...
public class Car : MonoBehaviour {
	...
	float CurrentEBRakeGripRatioFront;
	float CurrentEBRakeGripRatioRear;
...
//				Debug.Log (string.Format ("interpolating {0},{1}", CurrentEBRakeGripRatioFront, CurrentEBRakeGripRatioRear));
//			} else {
//				CurrentEBRakeGripRatioRear = 1.0f;
//				CurrentEBRakeGripRatioFront = 1.0f;
			}
		...
		if (Mathf.Abs (LocalAcceleration.y) > 18 || EBrake == 1) {
			...
			AxleFront.TireRight.SetTrailActive (true);
			AxleFront.TireLeft.SetTrailActive (true);
		} else {
